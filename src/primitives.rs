...
pub(crate) fn quintuple_round(state: &mut [u32; 4], rkey: [u8; 10], rconst: [u8; 5]) {
    sbox(&mut state[0], &mut state[1], &mut state[2], &mut state[3]);			
	...
	sbox(&mut state[3], &mut state[1], &mut state[2], &mut state[0]);			
	...
	swapmove(&mut state[1], &mut state[1], 0x55555555, 1);			
	swapmove(&mut state[2], &mut state[2], 0x00005555, 1);			
	swapmove(&mut state[3], &mut state[3], 0x55550000, 1);			
